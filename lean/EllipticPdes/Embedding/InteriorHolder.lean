...
* `d = 3`. Morrey needs `p > 3`, and the interior `H²` estimate supplies second derivatives in
  `L²`. The Gagliardo-Nirenberg-Sobolev bootstrap of `EllipticPdes.Embedding.GagliardoNirenberg`
  raises the gradient from `L²` to `L⁶`, and Morrey then applies at `p = 6 > 3 = d` with
  exponent `1 - 3/6 = 1/2`.

`d = 2` is left open. At `n = 2` and `p = 2` the sharp Gagliardo-Nirenberg-Sobolev inequality
fails, since `p < n` is false, so the route taken here does not reach it.
