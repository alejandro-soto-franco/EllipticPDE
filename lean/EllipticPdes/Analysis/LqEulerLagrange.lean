...
/-!
# Euler-Lagrange equation of a norm minimiser under an `L^q` constraint

A vector `U` of a real inner product space minimising `‖·‖` over `{W : ‖T W‖_{L^q} = 1}`, for a
continuous linear `T` into `L^q(μ)` with `q > 1`, satisfies

`⟪U, V⟫ = ‖U‖² ∫ |TU|^{q-2} (TU) (TV)`

for every `V`. The multiplier is `‖U‖²`, so no unknown constant survives.

The argument is Fermat's theorem applied to
`g(t) = ‖U + tV‖² - ‖U‖²‖T(U + tV)‖²_{L^q}`, which vanishes at `t = 0` and is nonnegative
everywhere: rescaling `U + tV` to the constraint set is admissible whenever its image is nonzero,
and where the image vanishes the second term does too.
...
The consumer is `EllipticPdes.Embedding.exists_minimiser_of_lt`, whose minimiser this equips with
an equation: on the unit ball, below the critical exponent, the minimiser of the `H₀¹` norm on the
unit `L^q` sphere is a weak solution of `-Δu + u = λ|u|^{q-2}u`.

## Main declarations

* `EllipticPdes.Analysis.norm_lp_rpow_eq_integral`: `‖f‖^q = ∫ |f|^q`.
* `EllipticPdes.Analysis.euler_lagrange_of_norm_min`: the equation.
...
theorem euler_lagrange_of_norm_min {p : ℝ≥0∞} [Fact (1 ≤ p)] (hp0 : p ≠ 0) (hptop : p ≠ ∞)
  ...
  set g : ℝ → ℝ := fun t => ‖U + t • V‖ ^ 2 - m ^ 2 * N t ^ (2 / r) with hgdef
  have hg0 : g 0 = 0 := by rw [hgdef]; simp [hN0, hmdef]
  have hgnonneg : ∀ t : ℝ, 0 ≤ g t := by
    ...
    · have hpos : 0 < ‖T (U + t • V)‖ := lt_of_le_of_ne (norm_nonneg _) (Ne.symm h0)
      ...
      rw [norm_smul, Real.norm_eq_abs, abs_of_pos (inv_pos.mpr hpos)] at hle
      have hmul : m * ‖T (U + t • V)‖ ≤ ‖U + t • V‖ := by
        calc m * ‖T (U + t • V)‖
            ≤ (‖T (U + t • V)‖⁻¹ * ‖U + t • V‖) * ‖T (U + t • V)‖ :=
              mul_le_mul_of_nonneg_right hle hpos.le
          _ = ‖U + t • V‖ := by field_simp
      have hsq := mul_self_le_mul_self (mul_nonneg (norm_nonneg U) hpos.le) hmul
      nlinarith [hsq]
  -- Fermat's theorem at the global minimum.
  have hlocmin : IsLocalMin g 0 :=
    Eventually.of_forall (fun t => by rw [hg0]; exact hgnonneg t)
  have hgderiv : HasDerivAt g (2 * ⟪U, V⟫ - m ^ 2 * (r * I * (2 / r) * N 0 ^ (2 / r - 1))) 0 := by
    have hE : HasDerivAt (fun t : ℝ => ‖U + t • V‖ ^ 2) (2 * ⟪U, V⟫) 0 := by
      have hpoly : HasDerivAt
          (fun t : ℝ => ‖U‖ ^ 2 + 2 * t * ⟪U, V⟫ + t ^ 2 * ‖V‖ ^ 2)
          (0 + 2 * 1 * ⟪U, V⟫ + 2 * 0 ^ 1 * ‖V‖ ^ 2) 0 :=
        (((hasDerivAt_const (0 : ℝ) (‖U‖ ^ 2)).add
          (((hasDerivAt_id (0 : ℝ)).const_mul 2).mul_const ⟪U, V⟫)).add
          ((hasDerivAt_pow 2 (0 : ℝ)).mul_const (‖V‖ ^ 2)))
      have hpoly' : HasDerivAt
          (fun t : ℝ => ‖U‖ ^ 2 + 2 * t * ⟪U, V⟫ + t ^ 2 * ‖V‖ ^ 2) (2 * ⟪U, V⟫) 0 := by
        convert hpoly using 1
        norm_num
      refine hpoly'.congr_of_eventuallyEq (Eventually.of_forall (fun t => ?_))
      change ‖U + t • V‖ ^ 2 = ‖U‖ ^ 2 + 2 * t * ⟪U, V⟫ + t ^ 2 * ‖V‖ ^ 2
      rw [← real_inner_self_eq_norm_sq, ← real_inner_self_eq_norm_sq,
        ← real_inner_self_eq_norm_sq, real_inner_add_add_self, real_inner_smul_right,
        real_inner_smul_left, real_inner_smul_right]
      ring
    have hNpow : HasDerivAt (fun t : ℝ => N t ^ (2 / r))
        (r * I * (2 / r) * N 0 ^ (2 / r - 1)) 0 :=
      hNderiv.rpow_const (Or.inl (by rw [hN0]; norm_num))
    exact hE.sub (hNpow.const_mul (m ^ 2))
