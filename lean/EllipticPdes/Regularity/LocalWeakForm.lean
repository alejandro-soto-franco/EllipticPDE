...
The derivation is mechanical. A test function supported in `V` has its graph in `H₀¹(Ω)`, so
the weak formulation applies to it; `EllipticCoeff.bilin_apply` and
`FullEllipticOp.lowerBilin_apply` expand the pairing into inner products; `inner_actL_eq` and
`inner_mulCoeffL_eq` turn each of those into an integral over `Ω`; each integrand carries a
...
/-! ### Bridge B1: the localised plain-integral weak identity -/

/-- **The localised weak formulation (Evans, *Partial Differential Equations* (2nd ed.),
§6.3.1, Theorem 2).** A weak solution `u ∈ H₀¹(Ω)` of `L u = f`, stated through the bilinear
pairing `Op.fullBilin` over the whole of `Ω`, satisfies the plain-integral identity
`∑_{i,j} ∫_V a_{ij}(∂ᵢu) ∂ⱼv + ∑_i ∫_V b_i (∂ᵢu) v + ∫_V c u v = ∫_V f v`
on any measurable `V ⊆ Ω`, for every test function `v` with `tsupport v ⊆ V`, with each
coordinate read as the `V`-restriction of its whole-space extension by zero. This is the shape
the differentiated-equation identity takes as its hypothesis `hLoc`. -/
...
/-! ### The differentiated-equation identity, from the weak formulation -/
...
second weak derivatives of `u` on `V` exist and are bounded by the data, and for every
direction `ℓ` in which the datum carries a weak derivative `Df`, the pair `(∂_ℓ∂ᵢu, Df)`
satisfies Evans' equation (34) against every test function supported in `V`.
