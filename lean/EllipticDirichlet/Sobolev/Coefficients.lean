...
def mulCoeffLM {Ω : Set (EuclideanSpace ℝ (Fin d))}
    {f : EuclideanSpace ℝ (Fin d) → ℝ} (hf : Measurable f) {M : ℝ}
    (hM : ∀ x, |f x| ≤ M) : L2D Ω →ₗ[ℝ] L2D Ω where
...
`∑ᵢⱼ aᵢⱼ(x) ξᵢ ξⱼ ≥ lam · |ξ|²` and `|aᵢⱼ(x)| ≤ Λ` for every `x`. This is exactly the data
...
structure EllipticCoeff (d : ℕ) where
  ...
  bdd : ∀ x i j, |a x i j| ≤ Λ
  elliptic : ∀ x (ξ : Fin d → ℝ), lam * ∑ i, ξ i ^ 2 ≤ ∑ i, ∑ j, a x i j * ξ i * ξ j
